...
import kr.co.koscom.openapitest.HttpConstants;
...
public class OAuthUtils {
	...
	public static OAuth2Details createOAuthDetails(Properties config) {
		...
		oauthDetails.setAccessToken((String) config
				.get(HttpConstants.ACCESS_TOKEN));
		oauthDetails.setRefreshToken((String) config
				.get(HttpConstants.REFRESH_TOKEN));
		oauthDetails.setGrantType((String) config
				.get(HttpConstants.GRANT_TYPE));
		oauthDetails.setClientId((String) config.get(HttpConstants.CLIENT_ID));
		oauthDetails.setClientSecret((String) config
				.get(HttpConstants.CLIENT_SECRET));
		oauthDetails.setScope((String) config.get(HttpConstants.SCOPE));
		oauthDetails.setAuthenticationServerUrl((String) config
				.get(HttpConstants.AUTHENTICATION_SERVER_URL));
		oauthDetails.setUsername((String) config.get(HttpConstants.USERNAME));
		oauthDetails.setPassword((String) config.get(HttpConstants.PASSWORD));
		oauthDetails.setResourceServerUrl((String) config
				.get(HttpConstants.RESOURCE_SERVER_URL));
		oauthDetails.setTokenEndpointUrl((String) config
				.get(HttpConstants.TOKEN_ENDPOINT_URL));
		oauthDetails.setRedirectURI((String) config
				.get(HttpConstants.REDIRECT_URI));
		oauthDetails.setState((String) config.get(HttpConstants.STATE));
...
//				.get(OAuthConstants.ACCESS_TYPE_VALUE));
	...
	public static String getAuthorizationCode(OAuth2Details oauthDetails) {
		...
		parametersBody.add(new BasicNameValuePair(HttpConstants.RESPONSE_TYPE,
				HttpConstants.CODE));

		parametersBody.add(new BasicNameValuePair(HttpConstants.CLIENT_ID,
				oauthDetails.getClientId()));

		parametersBody.add(new BasicNameValuePair(HttpConstants.REDIRECT_URI,
				oauthDetails.getRedirectURI()));

		if (isValid(oauthDetails.getScope())) {
			parametersBody.add(new BasicNameValuePair(HttpConstants.SCOPE,
		...
		if (isValid(state)) {
			parametersBody.add(new BasicNameValuePair(HttpConstants.STATE,
		...
		try {
			...
			Map<String, String> map = handleURLEncodedResponse(response);
                    ...
                    //    if (OAuthConstants.HTTP_OK == code) return null;
			if (HttpConstants.HTTP_SEND_REDIRECT == code) {
				location = getHeader(response.getAllHeaders(),
						HttpConstants.LOCATION_HEADER);
				if (location == null) {
					System.out
	...
	public static Map<String, String> getProtectedResource(
		...
		get.addHeader(HttpConstants.AUTHORIZATION,
		...
		try {
			...
			if (code == HttpConstants.HTTP_UNAUTHORIZED
					|| code == HttpConstants.HTTP_FORBIDDEN) {
				...
				if (isValid(accessToken)) {
					...
					get.removeHeaders(HttpConstants.AUTHORIZATION);
					get.addHeader(HttpConstants.AUTHORIZATION,
							getAuthorizationHeaderForAccessToken(oauthDetails
									.getAccessToken()));
	...
	public static Map<String, String> getAccessToken(
		...
		parametersBody.add(new BasicNameValuePair(HttpConstants.GRANT_TYPE,
				oauthDetails.getGrantType()));

		parametersBody.add(new BasicNameValuePair(HttpConstants.CODE,
				authorizationCode));
		...
		if (isValid(clientSecret)) {
			parametersBody.add(new BasicNameValuePair(
					HttpConstants.CLIENT_SECRET, clientSecret));
		}
	...
	public static Map<String, String> refreshAccessToken(
		...
		parametersBody.add(new BasicNameValuePair(HttpConstants.REFRESH_TOKEN,
	 ...
	 * @return
	 */
	public static Map handleResponse(String response) {
		String contentType = HttpConstants.JSON_CONTENT;
//		if (response.getEntity().getContentType() != null) {
//			contentType = response.getEntity().getContentType().getValue();
//		}
		if (contentType.contains(HttpConstants.JSON_CONTENT)) {
			return handleJsonResponse(response);
//		} else if (contentType.contains(HttpConstants.URL_ENCODED_CONTENT)) {
//			return handleURLEncodedResponse(response);
//		} else if (contentType.contains(HttpConstants.XML_CONTENT)) {
//			return handleXMLResponse(response);
	...
	public static Map handleJsonResponse(String response) {
	...
	public static String getAuthorizationHeaderForAccessToken(String accessToken) {
		return HttpConstants.BEARER + " " + accessToken;
	}

	public static String getBasicAuthorizationHeader(String username,
			String password) {
		return HttpConstants.BASIC + " "
...
//		}
		...
		if (!isValid(input.getRedirectURI())) {
			...
			invalidProps.add(HttpConstants.REDIRECT_URI);
		}
